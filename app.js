...
const writeUserData = (itemId, itemData, keywords, firebase) => {
    ...
    item.once('value', listing => {
        ...
        console.log(`successfuly writed id:${itemData.itemId[0]} price:${itemData.sellingStatus[0].currentPrice[0].__value__} ${itemData.title[0]} ${itemData.viewItemURL[0]}`);
    });
}

console.log('start!!');

const processItem = (item, keywords, firebase) => {
    ...
    writeUserData(itemId, item, keywords, firebase);
}

const getList = async (keywords, categoryId, filters='', sort='', appId='', firebase) => {
    ...
    const endpoint = 'http://svcs.ebay.com/services/search/FindingService/v1?OPERATION-NAME=findItemsAdvanced&SERVICE-VERSION=1.0.0&SECURITY-APPNAME='+appId+'&RESPONSE-DATA-FORMAT=JSON&REST-PAYLOAD&keywords='+keywords+'&categoryId='+categoryId+filters+sort+'&paginationInput.entriesPerPage=100';
...
// getList('T460S', categoryId, filters, sort);
// getList('T470S', categoryId, filters, sort);
// setInterval(() => {
//     getList('T460S', categoryId, filters, sort);
//     getList('T470S', categoryId, filters, sort);
